...
def update_positions(Vehicles, Implements, Tasks, Asignments, step_fraction, Vl, reached_implements, reached_tasks, Z_vehicles,Distancia,probabilityTA,probabilityTD,probabilityVA,probabilityVD,probabilityID,probabilityIA,totalDistancia):
    global check
    Event = [False, "", 0]
    New_task = []
    ...
    for i in range(num_vehicles):
        ...
        if not reached_implements[A_vehicles[i]]:
            ...
            totalDistancia[A_vehicles[i]]+=abs(Vl[i]*math.tan(alpha))
    ...
    for i in range(num_vehiclesd):
        ...
        totalDistancia[Z_vehicles[i]]+=abs(Vl[i]*math.tan(alpha))
    ...
    return Event, Vehicles, Implements, reached_info, New_task,Distancia,totalDistancia
...
def update_plot(ax, Vehicles, Implements, Tasks, Asignments, step_fraction, Vl, reached_implements, reached_tasks, Z_vehicles, Distancia, probabilityTA, probabilityTD, probabilityVA, probabilityVD, probabilityID, probabilityIA,totalDistancia):
    ax.clear()
    
    Event, updated_vehicles, updated_implements, reached_info, New_task, Distancia,totalDistancia = update_positions(Vehicles, Implements, Tasks, Asignments, step_fraction, Vl, reached_implements, reached_tasks, Z_vehicles, Distancia, probabilityTA, probabilityTD, probabilityVA, probabilityVD, probabilityID, probabilityIA,totalDistancia)
    ...
    return Event, updated_vehicles, updated_implements, reached_info, New_task,totalDistancia


def animate_allocation(Implements, Tasks, Vehicles, Asignments, ZAsignments,probabilityTA,probabilityTD,probabilityVA,probabilityVD,probabilityID,probabilityIA,jota,totalDistancia,num_steps=5):
    global check
    ...
    def animate(i):
        nonlocal execution_time,Tasks,jota,totalDistancia # Para modificar las variables en el ámbito externo
        global Event,New_task,check
        step_fraction = ((i+1) / num_steps)+jota 
        Event, updated_vehicles, updated_implements, reached_info, New_task,totalDistancia = update_plot(ax, Vehicles, Implements, Tasks, Asignments, step_fraction, Vl, reached_implements, reached_tasks, Z_vehicles,Distancia,probabilityTA,probabilityTD,probabilityVA,probabilityVD,probabilityID,probabilityIA,totalDistancia)
