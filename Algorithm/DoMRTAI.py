    ...
    while (len(Tasks) > 0) and (t < T):
        ...
        if Event[0]:  # Process completed assignments and events  
            ...
            try:
                ...
                Implements,Tasks,Vehicles,M,That=pop.UpdateInfoST(XAsignments,Implements,Tasks,Vehicles,M,That,b,ZAsignments,T_max,Distancia)
            except:
                # Time-extended model update: complex battery dynamics
                Implements,Tasks,Vehicles,M,That=pop.UpdateInfoTE(XAsignments,Implements,Tasks,Vehicles,M,That,num_periods,ZAsignments,b,T_max,TAsignments,num_vehicles)
        ...
        t=t+(tmo)
            ...
            # ==========================
            # 2.4.2 Event Processing - Handle dynamic environment changes
            # ==========================
        ...
        if len(K)==0:
            # All tasks completed - extract final efficiency values
            aTasck=Tasks[:,2]
            EfImplement=Implements[:,2]
            EfVehicle=Vehicles[:,2]
            Tasks=[]  # Clear task list to exit loop
        
        if t>T:
            # Time horizon exceeded - extract final efficiency values
            aTasck=Tasks[:,2]
            EfImplement=Implements[:,2]
            EfVehicle=Vehicles[:,2]
