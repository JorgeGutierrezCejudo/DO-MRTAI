...
def Optimization (C,M,That,I,K,V,Mmax,Cmax,IK,KI,IV,VI,KV,VK,alpha,beta,T_max,b,Tau,Vhat,Ihat,Khat,Cprime,Tmin):
    ...
    #Objective function
    obj = quicksum(quicksum(Cprime[t][v]* z[v,t]+ quicksum(C[t][i][k][v] * x[i, k, v, t] for i in I for k in K ) for v in V) + quicksum(M[t][k] * (o[k,t] - y[k,t])for k in K) for t in Tau)
